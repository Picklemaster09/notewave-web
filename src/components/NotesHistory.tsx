...
export function MarkdownViewer({ content }: MarkdownViewerProps) {
  ...
  const flushList = (key: number) => {
    ...
    currentList = null;
    return (
      <ul key={`list-${key}`} className="list-disc pl-5 my-2 space-y-1 text-gray-700">
        {list.items.map((item, i) => (
          <li key={i} className="text-xs font-semibold leading-relaxed">
            {parseInline(item)}
          </li>
        ))}
      </ul>
  ...
  const parseInline = (inlineText: string): React.ReactNode[] => {
    ...
    while (remaining.length > 0) {
      ...
      if (boldIdx !== -1 && (italicIdx === -1 || boldIdx <= italicIdx)) {
        ...
        if (nextBoldIdx !== -1) {
          const innerContent = remaining.substring(boldIdx + 2, nextBoldIdx);
          parts.push(<strong key={index++} className="font-black text-[#1C1C1E]">{innerContent}</strong>);
        ...
        if (nextItalicIdx !== -1) {
          const innerContent = remaining.substring(italicIdx + 1, nextItalicIdx);
          parts.push(<em key={index++} className="italic text-gray-650">{innerContent}</em>);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("### ")) {
      ...
      elements.push(
        <h4 key={`h3-${i}`} className="text-xs font-extrabold text-[#1C1C1E] mt-3 mb-1 tracking-tight block">
      ...
      if (listEl) elements.push(listEl);
      elements.push(
        <h3 key={`h2-${i}`} className="text-xs font-black text-[#1C1C1E] mt-4 mb-1.5 tracking-tight block border-b border-gray-100 pb-0.5">
      ...
      if (listEl) elements.push(listEl);
      elements.push(
        <h2 key={`h1-${i}`} className="text-[13px] font-black text-gray-900 mt-5 mb-2 tracking-tight block">
      ...
      if (trimmed === "") {
        ...
        elements.push(
          <p key={`p-${i}`} className="text-xs leading-relaxed text-gray-700 font-semibold mb-1.5">
            {parseInline(line)}
          </p>
        );
      }
          ...
          >
            ...
            {note.ideaSummary && (
              <div className="flex flex-col gap-1.5 bg-yellow-50/40 border border-yellow-200/50 p-4 rounded-xl">
                ...
                <div className="text-xs text-gray-750 leading-relaxed font-semibold">
                  <MarkdownViewer content={note.ideaSummary} />
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between gap-4 pt-3.5 border-t border-[#E5E5EA]">
              ...
              <button
                ...
                className="text-[10.5px] font-bold text-red-600 bg-red-50 hover:bg-red-600 hover:text-white border border-red-200 hover:border-red-600 px-3 py-1.5 rounded-lg transition-all cursor-pointer flex items-center gap-1.5 shadow-3xs"
              >
